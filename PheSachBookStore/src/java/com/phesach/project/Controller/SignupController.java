...
public class SignupController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            UserDTO newAccount = null;

            if (!password.equals(repassword)) {
                request.setAttribute("msg", "password not match");
                url = "register.jsp";
                request.getRequestDispatcher(url).forward(request, response);
            } else if (dao.checkUserExist(email) != null) {
                request.setAttribute("msg", "Account exist");
                url = "register.jsp";
                request.getRequestDispatcher(url).forward(request, response);
            } else {
                newAccount = new UserDTO(username, password, false, fullname, email);
